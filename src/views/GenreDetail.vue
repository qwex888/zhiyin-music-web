...
/** 未分类全量拉取：后端单页上限 1000，分页循环直至拉完 */
const UNCATEGORIZED_PAGE_SIZE = 1000;
let fetchId = 0;
...
/** 未分类：分页循环拉完全部歌曲，便于全选与批量改风格 */
const fetchAllUncategorizedSongs = async (currentId: number) => {
  ...
  while (true) {
    const { data } = await genresApi.uncategorizedSongs({
      limit: UNCATEGORIZED_PAGE_SIZE,
      offset,
    });
  ...
  if (genre.value) genre.value.song_count = total || allSongs.length;
};
...
const fetchData = async () => {
  ...
  try {
    if (isUncategorized.value) {
      await fetchAllUncategorizedSongs(currentId);
    } else {
...
<template>
  <div class="h-full">
    <div class="flex flex-col h-full overflow-hidden px-4 md:px-8 pt-4 md:pt-6 md:pb-8">
      <div class="flex-none space-y-4">
        ...
        <header v-else-if="genre" class="flex items-start gap-4 pb-4">
      ...
      <div v-if="!isLoading && genre" class="flex-1 overflow-hidden min-h-0">
        <VirtualSongList
          ref="listRef"
          :songs="songs"
          :is-loading="false"
